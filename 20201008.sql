...
EXCEPTION
    ...
    WHEN TOO_MANY_ROWS THEN DBMS_OUTPUT.PUT_LINE('TOO_DATA_ROWS');
END;
/
...
분포도 : 흩어진 정도
데이터 분포도 : 특정 데이터가 발생한는 빈도
예 : emp테이블의 empno : 값이 14개 - 데이터 분포도가 좋다 (조인)
    emp테이블의 empno : 값이 3개 존재 - 데이터 분포도가 나쁘다 (함수)
    
함수의 경우 오라클에서 기본적으로 캐쉬 기능을 사용
기본 캐쉬 사이즈가 20개
getDeptName(10) => ACCOUNTING 오라클은 10번인자로 getDeptName 실행 했을 때 ACCOUNTING이라는 결과를 기억(캐싱)
...
/
CREATE OR REPLACE TRIGGER trg_user_pass
    BEFORE UPDATE ON users
    FOR EACH ROW
    
BEGIN
    --테이블 업데이트가 일어 났을때
    --기존(:OLD.pass) 비밀번호와 업데이트 하려고 하는 비밀번호가(:NEW.pass) 다를때
...
users_history 에는 비번이 없느상황
SELECT *
FROM users_history;

DELETE users_history;
