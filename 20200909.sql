...
NULL 관련 함수 - NULL과 관련의 연산의 결과는 NULL
...
comn 컬럼이 NULL일때 0으로 변경하여 sal 컬럼과 합계를 구한다
...
SELECT userid, usernm, reg_dt, NVL(reg_dt, SYSDATE) n_reg_dt, 
       NVL2(reg_dt, reg_dt, SYSDATE) n_reg_dt2,
       coalesce(reg_dt, SYSDATE) n_reg_dt3
FROM users
WHERE userid NOT IN('brown'); : 부정형
in - list : 1000개까지만
WHERE userid IN('cony','sally','james','moon'); : 부정형
...
emp 테이블에서 job 컬럼의 값이 
    ...
    그밖의 직군('CLERK','ANALYST')dms sal 값에 그대로 반환
...
가변인자 : 인자의 개수가 정해지지 아님
DECODE(col|expr1,
                search1, return1,
                search1, return2,
                search1, return3,
                [default])
첫번째 컬럼이 두번째 컬럼(search1)과 같으면 세번째(return1) 컬럼을 리턴
첫번째 컬럼이 네번째 컬럼(search2)과 같으면 다섯번째(return2) 컬럼을 리턴
첫번째 컬럼이 여섯번째 컬럼(search3)과 같으면 일곱번째(return3) 컬럼을 리턴
일치하는 값이 없을 떄는 default 리턴
...
cond2]
직원의 입사년도와, 올해년도의 짝수 구분을 이용해서 올해 검강검진 대상자인지 구하는게 문제.

건강검진 대상 여부 : 출생년도의 짝수 구분과, 검강건진 실시년도(올해)의 짝수 구분이 같을 때
    ex) 1983년생은 홀수년도 출생이므로 2020년도(짝수년도) 에는 검강검진 비대상
        1983년생은 홀수년도 출생이므로 2021년도(홀수년도) 에는 검강검진 대상
...
SELECT empno, ename, hiredate,
       case
            ...
            ELSE '건강검진 비대상자'
       end conatact_to_doctor
FROM emp;


SELECT empno, ename ,hiredate,
       case
            WHEN a = b THEN '건강검진 대상자'
            ELSE '건강검진 비대상자'
       end contact_to_doctor
FROM(SELECT empno, ename, hiredate,
       case
            WHEN MOD(TO_CHAR(hiredate,'YY'),2) = 0 THEN 0
            ELSE 1
       end a,
       case
            WHEN MOD(TO_CHAR(SYSDATE,'YY'),2) = 0 THEN 0
            ELSE 1
       end b
FROM emp);
