...
DEPT 테이블의 모든부서에 대해 부서번호, 부서이름을 CURSOR를 통해 데이터 출력
...
파라미터가 있는 커서 : 함수처림 커서에 인자를 전달해서 실행시간에 조건을 추가 할 수 있다.
    FROM emp
    WHERE deptno = 10;
...
emo 테이블의 특정 부서에 속하는 사원들을 조회할 수 있는 커서를
...
BEGIN
    ...
    LOOP
        ...
        FETCH c_dt1 INTO v_dt1;
        EXIT WHEN c_dt1%NOTFOUND;
        v_sum :=  v_sum + v_dt-v_dt1;
    END LOOP;  
    avgdt := v_sum/c_dt1%ROWCOUNT; 
    CLOSE c_dt;
    CLOSE c_dt1;
    DBMS_OUTPUT.PUT_LINE(avgdt);
END;
/
EXEC avgdt;
...
SELECT AVG(dt)
FROM
(SELECT dt - LEAD(dt) over(ORDER BY dt DESC) dt
FROM dt);

SELECT (MAX(dt)-MIN(dt))/(COUNT(*)-1) dt
FROM dt;
