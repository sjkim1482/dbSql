...
비교연산 <, >, =, !=, <>, <=, >=
        BETWEEN AND
        IN
연산자를 배울 때 (복습할 때) 기억한 부분은
해당 연산자가 x항 연산자 인지를 인지하자
...
(조건식이 참이기 때문에 모든 정보가 다나옴)


NULL 비교
NULL 값은 =, != 등의 비교연산으로 비교가 불가능
EX : emp 테이블에는 comn컬럼의 값이 NULL인 데이터가 존재
comm인 NULL인 데이터를 조회 하기 위해 다음과 같이 실행할 경우
정상적으로 동작하지 않음
SELECT *
FROM emp
WHERE comm = NULL;
...
지금은 9369, 7499....조회가 되지만
내일 동일한 sql을 실행 하더라도 오늘 나온 순서가 보장되지 않는다. (바뀔수 있음)
...
SELECT empno eno, ename enm, empno+30
FROM emp
ORDER BY empno + 30;
...
SELECT *
FROM emp
WHERE comm IS NOT NULL AND comm != 0 //NULL은 비교가 안됨
