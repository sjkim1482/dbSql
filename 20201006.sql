...
복합변수
1. %ROWTYPE 행 정보를(컬럼이 복수) 담을 수 있는 변수(rowtype)
...
해당 타입의 변수를 선언하여 사번이 7369번인 사우너의 위 3가지 컬럼정보를 담아본다
...
위에 세가지 조건을 만족하지 못할 때는 sal*1.6값을
v_sal 변수에 담아 콘솔에 출력
...
DECLARE
    ...
    v_sal NUMBER;
BEGIN
    SELECT sal INTO v_sal7369
    FROM emp
    WHERE empno = 7369;
    
  v_sal:= CASE
    WHEN v_sal7369 > 1000 THEN v_sal7369*1.2
    WHEN v_sal7369 > 900 THEN v_sal7369*1.3
    WHEN v_sal7369 > 800 THEN v_sal7369*1.4
    ELSE v_sal7369*1.6
    END;
