많이 쓰이는 함수, 잘 알아두자
(개념적으로 혼돈하지 말고 잘 정리하자 - SELECT 절에 올수 있는 컬럼에 대해 잘 정리)

그룹함수 : 여러게의 행을 입력으로 받아 하나의 행으로 결과를 반환하는 함수
...
COUNT(컬럼|익스프레션|*(카운터는 *를 쓸수있음)) : 그룹핑된 행의 개수
...
질문] GROUP BY 를 사용하면 WHERE 절을 사용못하냐?
...
grp1]
SELECT MAX(sal) MAX_SAL, MIN(sal) min_sal, ROUND(avg(sal),2) age_sal, SUM(sal) sum_sal , count(sal) count_sal, count(mgr) count_mgr, count(*) count_all
FROM emp;

grp2]
SELECT deptno, MAX(sal) MAX_SAL, MIN(sal) min_sal, ROUND(avg(sal),2) age_sal, SUM(sal) sum_sal , count(sal) count_sal, count(mgr) count_mgr, count(*) count_all
FROM emp
GROUP BY deptno;
...
SELECT CASE
        ...
        END dname, 
        MAX(sal) MAX_SAL, MIN(sal) min_sal, ROUND(avg(sal),2) age_sal, SUM(sal) sum_sal ,
        count(sal) count_sal, count(mgr) count_mgr, count(*) count_all,
        DECODE(deptno, 10 ,'ACCOUNTING',20 , 'RESERACH',  30 , 'SALES',40 ,'OPERATIONS') dname2
FROM emp 
GROUP BY deptno
ORDER BY dname;

SELECT DECODE(deptno, 10 ,'ACCOUNTING',20 , 'RESERACH',  30 , 'SALES') dname2,
        MAX(sal) MAX_SAL, MIN(sal) min_sal, ROUND(avg(sal),2) age_sal, SUM(sal) sum_sal ,
        count(sal) count_sal, count(mgr) count_mgr, count(*) count_all
FROM emp 
GROUP BY DECODE(deptno, 10 ,'ACCOUNTING',20 , 'RESERACH',  30 , 'SALES');
ORDER BY dname;
...
JOIN : 다른 테이블과 연결하여 데이터를 확장한는 문법
        . 컬럼을 확장
...
JOIN 문법
  1. ANSI - SQL
        : RDBMS에서 사용하는 SQL 표준
          ( 표준을 잘 지킨 모든 RDBMS-MYSQL, MSSQL, POSTGERSQL... 에서 실행가능 )
  2. ORACLE - SQL
        : ORACLE 사만의 고유 문법
...
NATURAL JOIN : 조인하고자 하는 테이블의 컬럼명이 같은 컬럼끼리 연결
    ...
    SELECT 컬럼
    FROM 테이블명 NATURAL JOIN 테이블명; 조인되는 컬럼은 . 를 안붙히고 조회해야함
...
오라클 입장에선는 해당 컬럼이 어떤 테이블의 컬럼인지 알 수가 없을 때 발생.
...
SELECT *
FROM emp e, dept d
WHERE e.deptno = d.deptno AND d.deptno IN(20,30);
...
FROM emp e , emp m 
WHERE e.mgr = m.empno;
KING의 경우 mgr 컬럼의 값이 NULL 이기 때문에 e.mgr = m.empno 조건을 충족시키지 못함
그래서 조인에 실패해서 14건중 13건의 데이터만 조회
