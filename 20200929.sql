...
이전/이후 n행 값 가져오기
LAG(col, [건너뛸 행 수 - default 1], [값이 없을경우 적용할 기본값])
SELECT empno, ename, hiredate, job, sal, 
...
실습 ana7] 급여정보를 부서별로, 급여, 사원번호 오름차순 정렬 했을 때
자신의 급여와 선행하는 사원들의 급여합(부서별)
...
*** 동일한 SQL 이란??
 ...
 - DBMS 입장에서는 완벽하게 문자열이 동일해야 동일한 SQL임
   ...
   4. Select /* SQL_TEST */ *  FROM emp WHERE deptno = :deptno;
...
LEVEL 2 : REPEATABLE READ (반복적 읽기)
          트랜잭션 안에서 동일한 SELECT 쿼리를 실행해도 트랜잭션의 어떤 위치에서 던지
