...
SELECT LEVEL, dummy
   ...
   SELECT /*day, d,8 DECODE(d, 1, iw+1, iw) ,*/ MIN(DECODE(d, 1, day)) 일, MIN(DECODE(d, 2, day)) 월, MIN(DECODE(d, 3, day)) 화,
                           MIN(DECODE(d, 4, day)) 수, MIN(DECODE(d, 5, day)) 목, MIN(DECODE(d, 6, day)) 금, 
                           MIN(DECODE(d, 7, day)) 토
   FROM
...
DECLARE
    선언부(생략가능)
          - PL/SQL 블럭에서 사용할 변수, TYPE(CLASS), CURSOR(SQL-정보)등을 선언하는 절
            JAVA랑은 다르게 변수선언을 블록 어디서나 할 수 있음
BEGIN
    실행부(생략불가)
...
PL/SQL 연산자 : SQL과 동일
               ...
               (변수)대입 연산자 주의(SQL에 존재하지 않음)
               JAVA =
               PL/SQL :=
        
10번 부서의 부서번호, 부서이름을 각 변수에 담아서 console에 출력
부서번호 : v_deptno, 부서이름 : n_deptnm
변수 선언 : java와 순서가 다름 ㅠ_ㅠ
    java : 타입 변수명
    pl/sql : 변수명 타입
...
BEGIN
    ...
    FROM dept
    WHERE deptno = 10;
    DBMS_OUTPUT.PUT_LINE('v_deptno : ' || v_deptno || 'V_dname');
END;
/
