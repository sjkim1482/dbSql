...
2. 문자 : VARCHAR2(사이즈-byte), CHAR(X)
         ...
         CHAR(5) 'test'
         test는 4byte고 char(5) 5byte이기 떄문에
...
4. FOREIGN KEY : 해당 걸럼의 참조하는 다른 테이블의 컬럼에 값이 존재해야하는 제약조건
                 emp.deptno ==> dept.deptno
5. CHECK : 컬럼에 들어갈 수 있는 값을 제한하는 제약조건
           ex) 성별이라는 컬림 있다고 가정
               들어갈수 있는 값이 : 남 M, 여 F ==> T ?? //NOT NULL 제약조건
...
CREATE TABLE dept_test (
    deptno NUMBER(2) [CONSTRAINT 제약조건이름] PRIMARY KEY,
    dname VARCHAR2(14),
    loc VARCHAR(13)
    );
...
unique constraint (SJK.SYS_C007084)[우리가 생성을 한 제약조건에 대한 이흠] violated(유니크 제약조건이 위배됨)
90번 부서가 이미 존재 하는 상태 였기 때문에 PRIMARY KEY 제약에 의해 정상적으로 입력될 수 없다.
INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');

비교
dept 테이블에는 deptno컬럼에 PRIMARY KEY 제약이 없는 상태
그렇게 때문에 deptno 컬럼의 값이 중복이 가능
...
CREATE TABLE dept_test (
    ...
    dname VARCHAR2(14),
    loc VARCHAR(13),
    CONSTRAINT PK_detp_test PRIMARY KEY (deptno, dname)
0);

CONSTRAINT PK_detp_test PRIMARY KEY (deptno, dname) 두개다 같으면 제약위배 하나라도 다르면 ㄱㅊ
deptno 컬럼의 값은 90으로 같지만 dname 컬럼의 값이 다르므로
PRIMARY KEY (deptnom dname) 설정에 따라 데이터가 입력될 수 있다.
...
emp_test : 90번 부서를 참조하는 9000번 brown 이 존재
...
2. 참조 무결성 생성시 OPTION - ON DELETE SET NULL
            삭제시 참조 하고 있는 자식테이블의 컬럼은 NULL로 만든다
...
90번 부서에 속한 9000번 사원의 데이터도 삭제된다.
SELECT *
FROM emp_test;
...
2. DDL은 AUTO COMMIT(DDL실행시 자동커밋)
DROP TABLE emp_test;
DROP TABLE dept_test;
...
테이블이 이미 생성된 시점에서 제약조건을 추가, 삭제하기
문법
